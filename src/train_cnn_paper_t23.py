...
# -----------------------
# Config (T23 paper-faithful su RML2016.10b)
# -----------------------
DATA_PATH = "data/RML2016.10b.dat"
MODEL_OUT = "models/cnn_paper_t23_10c.keras"
TRAIN_METRICS_OUT = "outputs/tables/cnn_paper_t23_train_metrics.json"
...
# GPU memory growth (evita allocazione aggressiva VRAM)
...
def build_stratified_splits(Xd, seed=42, num_classes=10, train_ratio=0.5):
    ...
    # shuffle globale per evitare ordinamenti per classe/SNR
...
def build_cnn_paper_t23(input_shape, num_classes):
    """
    T23 (Table IV):
      Conv(32,  1x3) -> Dropout(0.5)
      Conv(64,  1x3) -> Dropout(0.5)
      Conv(64,  1x3) -> Dropout(0.5)
      Conv(128, 2x3) -> Dropout(0.5)
      Dense(128, relu) -> Dropout(0.5)
      Dense(num_classes, softmax)
    """
    inp = layers.Input(shape=input_shape)

    x = layers.Conv2D(32, (1, 3), padding="same", activation="relu")(inp)
...
def main():
    ...
    model = build_cnn_paper_t23(X_train.shape[1:], NUM_CLASSES)
    model.compile(
    ...
    # Nota: paper cita best validation loss -> monitoriamo val_loss
    cbs = [
        ReduceLROnPlateau(
            monitor="val_loss",
            factor=0.5,
    ...
    print(f"[CNN_PAPER_T23] Test accuracy: {test_acc:.4f} | Test loss: {test_loss:.4f}")
