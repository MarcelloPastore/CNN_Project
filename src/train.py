...
# -----------------------
# Config (CNN2 paper-style)
# -----------------------
DATA_PATH = "data/RML2016.10b.dat"
MODEL_OUT = "models/cnn2_10c.keras"
TRAIN_METRICS_OUT = "outputs/tables/cnn2_train_metrics.json"
...
def build_cnn2(input_shape, num_classes):
    inp = layers.Input(shape=input_shape)

    x = layers.Conv2D(256, (1, 3), padding="same", activation="relu")(inp)
    x = layers.Dropout(0.5)(x)

    x = layers.Conv2D(80, (2, 3), padding="same", activation="relu")(x)
    x = layers.Dropout(0.5)(x)
    ...
    return keras.Model(inp, out, name="cnn2")
...
def main():
    ...
    model = build_cnn2(input_shape=X_train.shape[1:], num_classes=NUM_CLASSES)
    model.compile(
    ...
    print(f"[CNN2] Test accuracy: {test_acc:.4f} | Test loss: {test_loss:.4f}")
    ...
    metrics = {
        ...
        "sec_per_epoch_avg": float(train_time_sec / max(1, len(hist.history["loss"]))),
